...
Window_BattleStatus.prototype.drawGaugeArea = function(rect, actor) {
    ...
    wymod *= 1.5
    this.drawActorHp(actor, rect.x, wy - wymod, rect.width-200);
    if (this.getGaugesDrawn(actor) <= 2) {
      this.drawActorMp(actor, rect.x, wy, rect.width);
    } else {
      var ww = rect.width / 2 - 100;
      this.drawActorMp(actor, rect.x, wy, ww-10);
      this.drawActorTp(actor, rect.x + ww + 10, wy, ww-10);
    }
...
Window_BattleStatus.prototype.gaugeAreaRect = function(index) {
    var rect = this.itemRectForText(index);
    rect.height = this.contents.height - this.lineHeight() * 4;
